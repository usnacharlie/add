...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" 
    ...
    def query_customer_payment(self, payment_reference):
        ...
        if result['success']:
            ...
            # First check if paymentStatus field exists
            cgrate_status = data.get('paymentStatus', '').upper()

            # If paymentStatus not present, check responseMessage
            if not cgrate_status or cgrate_status == '':
                response_message = data.get('responseMessage', 'UNKNOWN').upper()
                if 'SUCCESSFUL' in response_message or 'SUCCESS' in response_message:
                    cgrate_status = 'SUCCESSFUL'
                elif 'FAILED' in response_message or 'FAIL' in response_message or 'ERROR' in response_message:
                    cgrate_status = 'FAILED'
                elif 'PENDING' in response_message or 'PROCESSING' in response_message:
                    cgrate_status = 'PENDING'
                else:
                    cgrate_status = 'UNKNOWN'
